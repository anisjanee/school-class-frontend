...
app = FastAPI(title="School Journal Pro API", version="3.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def migrate_users_table(conn):
    ...
    conn.execute(
        """INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at)
           SELECT id,role,name,email,password_hash,student_id,active,created_at FROM users_legacy"""
    )
...
def init_db():
    ...
    for table, column, definition in [
        ...
        ("students", "phone", "TEXT DEFAULT ''"),
        ("students", "created_at", "TEXT NOT NULL DEFAULT ''"),
        ("subjects", "teacher_id", "TEXT"),
        ("subjects", "created_at", "TEXT NOT NULL DEFAULT ''"),
    ]:
        add_column_if_missing(conn, table, column, definition)

    stamp = now_iso()
    conn.execute("UPDATE users SET created_at=? WHERE created_at=''", (stamp,))
    conn.execute("UPDATE students SET created_at=? WHERE created_at=''", (stamp,))
    conn.execute("UPDATE subjects SET created_at=? WHERE created_at=''", (stamp,))

    if not conn.execute("SELECT 1 FROM users WHERE lower(email)=?", ("admin@school.local",)).fetchone():
        conn.execute(
            "INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at) VALUES(?,?,?,?,?,?,?,?)",
            ("admin", "admin", "Администратор", "admin@school.local", hash_password("Admin123!"), None, 1, stamp),
        )

    if not conn.execute("SELECT 1 FROM users WHERE email=?", ("teacher@demo.local",)).fetchone():
        conn.execute(
            "INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at) VALUES(?,?,?,?,?,?,?,?)",
            ("t1", "teacher", "Учитель Demo", "teacher@demo.local", hash_password("1234"), None, 1, stamp),
        )
    if not conn.execute("SELECT 1 FROM students WHERE id=?", ("s1",)).fetchone():
        conn.execute("INSERT INTO students(id,name,class_name,phone,created_at) VALUES(?,?,?,?,?)", ("s1", "Ученик Demo", "10-А", "", stamp))
    if not conn.execute("SELECT 1 FROM users WHERE email=?", ("student@demo.local",)).fetchone():
        conn.execute(
            "INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at) VALUES(?,?,?,?,?,?,?,?)",
            ("s1", "student", "Ученик Demo", "student@demo.local", hash_password("1234"), "s1", 1, stamp),
        )
    if not conn.execute("SELECT 1 FROM users WHERE email=?", ("parent@demo.local",)).fetchone():
        conn.execute(
            "INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at) VALUES(?,?,?,?,?,?,?,?)",
            ("p1", "parent", "Родитель Demo", "parent@demo.local", hash_password("1234"), "s1", 1, stamp),
        )
    if not conn.execute("SELECT 1 FROM subjects WHERE id=?", ("math",)).fetchone():
        teacher = conn.execute("SELECT id FROM users WHERE role='teacher' ORDER BY created_at LIMIT 1").fetchone()
        tid = teacher[0] if teacher else None
        conn.execute("INSERT INTO subjects(id,name,teacher_id,created_at) VALUES(?,?,?,?)", ("math", "Математика", tid, stamp))
    if not conn.execute("SELECT 1 FROM subjects WHERE id=?", ("eng",)).fetchone():
        teacher = conn.execute("SELECT id FROM users WHERE role='teacher' ORDER BY created_at LIMIT 1").fetchone()
        tid = teacher[0] if teacher else None
        conn.execute("INSERT INTO subjects(id,name,teacher_id,created_at) VALUES(?,?,?,?)", ("eng", "Английский", tid, stamp))
    if conn.execute("SELECT COUNT(*) FROM grades").fetchone()[0] == 0:
        conn.execute("INSERT INTO grades(student_id,subject_id,value,comment,created_at) VALUES(?,?,?,?,?)", ("s1", "math", 9, "", stamp))
        conn.execute("INSERT INTO grades(student_id,subject_id,value,comment,created_at) VALUES(?,?,?,?,?)", ("s1", "eng", 8, "", stamp))
    conn.commit()
    conn.close()
...
class StudentIn(BaseModel):
    name: str = Field(min_length=2, max_length=100)
    class_name: str = Field(default="10-А", min_length=1, max_length=30)
...
def student_filter_sql(user, prefix=""):
    ids = visible_student_ids(user)
    if ids is None:
        return "", []
    if not ids:
        return " AND 1=0", []
    return f" AND {prefix}student_id=?", [ids[0]]


def get_students_for_user(conn, user):
    if user["role"] in {"admin", "teacher"}:
        return [dict(r) for r in conn.execute("SELECT * FROM students ORDER BY class_name,name").fetchall()]
    sid = user.get("student_id")
    if not sid:
        return []
    return [dict(r) for r in conn.execute("SELECT * FROM students WHERE id=?", (sid,)).fetchall()]


@app.get("/")
def root():
    return {"name": "School Journal Pro API", "status": "ok", "version": "3.0.0", "docs": "/docs"}
...
def health():
    ...
    conn.close()
    return {"status": "ok", "database": DB_PATH.name, "users": users, "version": "3.0.0"}
...
def login(payload: LoginIn):
    ...
    conn.execute("INSERT INTO sessions(token,user_id,expires_at) VALUES(?,?,?)", (token, user["id"], expires.isoformat()))
    conn.commit()
    conn.close()
...
def register(payload: RegisterIn):
    ...
    uid = "u_" + secrets.token_hex(7)
    sid = payload.student_id
    if role == "student":
        sid = "s_" + secrets.token_hex(7)
    ...
    elif sid and not conn.execute("SELECT 1 FROM students WHERE id=?", (sid,)).fetchone():
        conn.close()
        raise HTTPException(404, "Ученик для привязки не найден")
    conn.execute(
        "INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at) VALUES(?,?,?,?,?,?,?,?)",
        (uid, role, payload.name.strip(), email, hash_password(payload.password), sid, 1, now_iso()),
    )
    conn.commit()
...
def dashboard(user=Depends(current_user)):
    conn = db()
    students = get_students_for_user(conn, user)
    ...
    if user["role"] in {"admin", "teacher"}:
        grades = [dict(r) for r in conn.execute("SELECT g.*,s.name student_name,sub.name subject_name FROM grades g JOIN students s ON s.id=g.student_id JOIN subjects sub ON sub.id=g.subject_id ORDER BY g.created_at DESC,g.id DESC LIMIT 300").fetchall()]
    else:
        grades = [dict(r) for r in conn.execute("SELECT g.*,s.name student_name,sub.name subject_name FROM grades g JOIN students s ON s.id=g.student_id JOIN subjects sub ON sub.id=g.subject_id WHERE g.student_id=? ORDER BY g.created_at DESC,g.id DESC LIMIT 300", (user.get("student_id"),)).fetchall()] if user.get("student_id") else []
    ...
    if user["role"] not in {"admin", "teacher"}:
        ...
        schedule = [x for x in schedule if x["class_name"] in classes]
        homework = [x for x in homework if x["class_name"] in classes]
    ...
    notifications = [dict(r) for r in conn.execute("SELECT id,text,created_at,author_id FROM notifications ORDER BY id DESC LIMIT 50").fetchall()]
    conn.close()
    return {
        "user": public_user(user),
        "students": students,
        "all_students": students if user["role"] in {"admin", "teacher"} else [],
        "subjects": subjects,
        "grades": grades,
        "users": users,
        "schedule": schedule,
...
def list_users(user=Depends(require_admin)):
    conn = db()
    rows = [public_user(r) for r in conn.execute("SELECT * FROM users ORDER BY role,name").fetchall()]
    conn.close()
    return rows
...
def create_user(payload: RegisterIn, user=Depends(require_admin)):
    ...
    if role not in ROLES - {"admin"}:
        raise HTTPException(400, "Администратор может создать учителя, ученика или родителя")
    email = payload.email.strip().lower()
    conn = db()
    if conn.execute("SELECT 1 FROM users WHERE lower(email)=?", (email,)).fetchone():
        conn.close()
    ...
    if role == "student":
        sid = "s_" + secrets.token_hex(7)
        conn.execute("INSERT INTO students(id,name,class_name,phone,created_at) VALUES(?,?,?,?,?)", (sid, payload.name.strip(), payload.class_name.strip() or "10-А", "", now_iso()))
    elif role == "parent" and sid:
        if not conn.execute("SELECT 1 FROM students WHERE id=?", (sid,)).fetchone():
            conn.close()
            raise HTTPException(404, "Ученик для привязки не найден")
    conn.execute("INSERT INTO users(id,role,name,email,password_hash,student_id,active,created_at) VALUES(?,?,?,?,?,?,?,?)", (uid, role, payload.name.strip(), email, hash_password(payload.password), sid, 1, now_iso()))
    conn.commit()
    row = conn.execute("SELECT * FROM users WHERE id=?", (uid,)).fetchone()
    conn.close()
...
def update_user(user_id: str, payload: UserUpdate, user=Depends(require_admin)):
    ...
    for key, value in [("name", payload.name), ("email", payload.email), ("role", payload.role), ("active", None if payload.active is None else int(payload.active)), ("student_id", payload.student_id)]:
        if value is not None:
            fields.append(f"{key}=?")
            values.append(value.strip().lower() if key == "email" else value)
    if payload.password:
        fields.append("password_hash=?")
        values.append(hash_password(payload.password))
    if payload.student_id and not conn.execute("SELECT 1 FROM students WHERE id=?", (payload.student_id,)).fetchone():
        conn.close()
        raise HTTPException(404, "Ученик для привязки не найден")
    if role == "student" and not target["student_id"]:
        sid = "s_" + secrets.token_hex(7)
        conn.execute("INSERT INTO students(id,name,class_name,phone,created_at) VALUES(?,?,?,?,?)", (sid, payload.name or target["name"], "10-А", "", now_iso()))
...
def list_students(user=Depends(current_user)):
    conn = db()
    rows = get_students_for_user(conn, user)
    conn.close()
    return rows
...
def create_student(payload: StudentIn, user=Depends(require_admin)):
    sid = "s_" + secrets.token_hex(7)
    conn = db()
    conn.execute("INSERT INTO students(id,name,class_name,phone,created_at) VALUES(?,?,?,?,?)", (sid, payload.name.strip(), payload.class_name.strip(), payload.phone.strip(), now_iso()))
    conn.commit()
    conn.close()
    return {"id": sid, "name": payload.name.strip(), "class_name": payload.class_name.strip(), "phone": payload.phone.strip()}


@app.patch("/api/students/{student_id}")
def update_student(student_id: str, payload: StudentIn, user=Depends(require_admin)):
    conn = db()
    cur = conn.execute("UPDATE students SET name=?,class_name=?,phone=? WHERE id=?", (payload.name.strip(), payload.class_name.strip(), payload.phone.strip(), student_id))
    conn.commit()
    conn.close()
    if cur.rowcount == 0:
...
def list_subjects(user=Depends(current_user)):
    conn = db()
    rows = [dict(r) for r in conn.execute("SELECT s.*,u.name teacher_name FROM subjects s LEFT JOIN users u ON u.id=s.teacher_id ORDER BY s.name").fetchall()]
    conn.close()
    return rows
...
def create_subject(payload: SubjectIn, user=Depends(require_admin)):
    sid = "sub_" + secrets.token_hex(7)
    conn = db()
    if payload.teacher_id and not conn.execute("SELECT 1 FROM users WHERE id=? AND role='teacher'", (payload.teacher_id,)).fetchone():
        conn.close()
        raise HTTPException(404, "Учитель не найден")
    conn.execute("INSERT INTO subjects(id,name,teacher_id,created_at) VALUES(?,?,?,?)", (sid, payload.name.strip(), payload.teacher_id, now_iso()))
    conn.commit()
    conn.close()
    return {"id": sid, "name": payload.name.strip(), "teacher_id": payload.teacher_id}
...
def update_subject(subject_id: str, payload: SubjectIn, user=Depends(require_admin)):
    ...
    if payload.teacher_id and not conn.execute("SELECT 1 FROM users WHERE id=? AND role='teacher'", (payload.teacher_id,)).fetchone():
        conn.close()
        raise HTTPException(404, "Учитель не найден")
    cur = conn.execute("UPDATE subjects SET name=?,teacher_id=? WHERE id=?", (payload.name.strip(), payload.teacher_id, subject_id))
    conn.commit()
    conn.close()
    if cur.rowcount == 0:
...
def create_grade(payload: GradeIn, user=Depends(require_staff)):
    ...
    cur = conn.execute("INSERT INTO grades(student_id,subject_id,value,comment,created_at) VALUES(?,?,?,?,?)", (payload.student_id, payload.subject_id, payload.value, payload.comment.strip(), now_iso()))
    conn.commit()
    gid = cur.lastrowid
    conn.close()
...
def update_grade(grade_id: int, payload: GradeIn, user=Depends(require_staff)):
    conn = db()
    cur = conn.execute("UPDATE grades SET student_id=?,subject_id=?,value=?,comment=? WHERE id=?", (payload.student_id, payload.subject_id, payload.value, payload.comment.strip(), grade_id))
    conn.commit()
    conn.close()
    if cur.rowcount == 0:
...
def list_schedule(user=Depends(current_user)):
    conn = db()
    rows = [dict(r) for r in conn.execute("SELECT sc.*,sub.name subject_name,u.name teacher_name FROM schedule sc JOIN subjects sub ON sub.id=sc.subject_id LEFT JOIN users u ON u.id=sc.teacher_id ORDER BY sc.day_of_week,sc.lesson_number").fetchall()]
    if user["role"] not in {"admin", "teacher"}:
        classes = {s["class_name"] for s in get_students_for_user(conn, user)}
...
def create_schedule(payload: ScheduleIn, user=Depends(require_admin)):
    ...
    if not conn.execute("SELECT 1 FROM subjects WHERE id=?", (payload.subject_id,)).fetchone():
        conn.close(); raise HTTPException(404, "Предмет не найден")
    if payload.teacher_id and not conn.execute("SELECT 1 FROM users WHERE id=? AND role='teacher'", (payload.teacher_id,)).fetchone():
        conn.close(); raise HTTPException(404, "Учитель не найден")
...
def create_homework(payload: HomeworkIn, user=Depends(require_staff)):
    ...
    if not conn.execute("SELECT 1 FROM subjects WHERE id=?", (payload.subject_id,)).fetchone():
        conn.close(); raise HTTPException(404, "Предмет не найден")
    teacher_id = user["id"] if user["role"] == "teacher" else (user["id"])
    cur = conn.execute("INSERT INTO homework(subject_id,class_name,title,description,due_date,teacher_id,created_at) VALUES(?,?,?,?,?,?,?)", (payload.subject_id,payload.class_name.strip(),payload.title.strip(),payload.description.strip(),payload.due_date,teacher_id,now_iso()))
...
def update_homework(homework_id: int, payload: HomeworkIn, user=Depends(require_staff)):
    ...
    if user["role"] == "teacher":
        allowed = conn.execute("SELECT 1 FROM homework WHERE id=? AND teacher_id=?", (homework_id,user["id"])).fetchone()
        if not allowed:
            conn.close(); raise HTTPException(403, "Можно изменять только свои задания")
    cur = conn.execute("UPDATE homework SET subject_id=?,class_name=?,title=?,description=?,due_date=? WHERE id=?", (payload.subject_id,payload.class_name.strip(),payload.title.strip(),payload.description.strip(),payload.due_date,homework_id))
    conn.commit(); conn.close()
...
def delete_homework(homework_id: int, user=Depends(require_staff)):
    ...
    if user["role"] == "teacher":
        allowed = conn.execute("SELECT 1 FROM homework WHERE id=? AND teacher_id=?", (homework_id,user["id"])).fetchone()
        if not allowed:
            conn.close(); raise HTTPException(403, "Можно удалить только свои задания")
...
def send_message(payload: MessageIn, user=Depends(current_user)):
    ...
    conn = db()
    receiver = conn.execute("SELECT id,active FROM users WHERE id=?", (payload.receiver_id,)).fetchone()
    if not receiver:
        conn.close(); raise HTTPException(404, "Получатель не найден")
    if not receiver["active"]:
        conn.close(); raise HTTPException(400, "Получатель отключён")
    cur = conn.execute("INSERT INTO messages(sender_id,receiver_id,text,created_at) VALUES(?,?,?,?)", (user["id"],payload.receiver_id,payload.text.strip(),now_iso()))
...
def create_notification(payload: NoticeIn, user=Depends(require_admin)):
    conn = db(); cur = conn.execute("INSERT INTO notifications(text,created_at,author_id) VALUES(?,?,?)", (payload.text.strip(),now_iso(),user["id"])); conn.commit(); nid = cur.lastrowid; conn.close()
    return {"id": nid, "ok": True}


@app.post("/api/sync")
def sync(user=Depends(current_user)):
    return {"ok": True, "server_time": now_iso(), "user": public_user(user)}
